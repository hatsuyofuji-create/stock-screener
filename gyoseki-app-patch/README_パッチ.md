...
## いちばん簡単なやり方（推奨・ファイルを2つ置くだけ）

このフォルダにある次の2ファイルを、**業績予想アプリのフォルダ**（`ir_scraper.py` が
入っているフォルダ）にコピーしてください。`ir_scraper.py` は上書きになります。

1. `ai_pdf_extract.py` … 新規追加
2. `ir_scraper.py` … 既存を上書き（=下の手動編集が済んだ状態のファイル）

これで完了です。手で編集する必要はありません。`ai_analyzer.py` は変更していないので、
そのまま残してください（フォールバックとして使われます）。

> ⚠️ 上書き前に、念のため元の `ir_scraper.py` をコピーしてバックアップを取っておくと安心です。

---

## （参考）自分で手編集したい場合の手順

「丸ごと上書きは不安、自分で直したい」という場合は、次の2ステップでも同じ結果になります。
