...
- [x] **フロントエンド** — React + TS + Tailwind + Recharts（マトリクス散布図・銘柄詳細・
      スクリーニング・ポートフォリオ・景気局面・予想Excel取込）→ [`frontend/`](frontend/README.md)

> Phase 6 は自動取得に依存しないため Phase 5 より先に実装（手動/自動どちらのデータでも動作）。

**仕様書の全6フェーズ＋フロントエンドが完了**。次の候補：EDINET(XBRL) による
財務三表の網羅取得／予想Excelのマッピング確定・保存UI／実データ疎通。
