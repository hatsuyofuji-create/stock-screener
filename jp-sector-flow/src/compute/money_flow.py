...
考え方（表示専用・売買判断はしない）:
  ...
  2. 売買代金は買い・売り両方の合計なので方向は分からない。方向は価格（終値）で見る：
     業種の等ウェイト株価指数の変化率を、5日・15日・30日・150日の4期間で出す。
     プラス＝買い優勢🟢／マイナス＝売り優勢🔴。
...
HORIZONS = [5, 15, 30, 150]  # 価格の方向を測る営業日数
...
def compute(
    ...
    戻り値 dict:
      ...
      - "ranking": DataFrame（rank, sector, turnover[億円], price_mom_5/15/30/150[%]）
